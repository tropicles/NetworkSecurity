...
class DataValidation:
    ...
    def validate_numerical_columnns(self,dataframe:pd.DataFrame)->bool:
        try:
            numerical_column_names=self.schema_config.get("numerical_columns")
            logging.info(f"Numerical column names:{numerical_column_names}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            status=self.validate_number_columns(train_dataframe)
            if not status:
            ...
            status=self.validate_numerical_columnns(train_dataframe)
            if not status:
                error_message=f"Train dataframe does not contain numerical columns"
            status=self.validate_numerical_columnns(test_dataframe)
            if not status:
